...
namespace BankIntegration.Repository
{
    public static class MainRepository
    {
        ...
        public static IList<RequestModel> GetAllRequests()
        {
            ...
            string qry = "SELECT * FROM Request WHERE Status=0";
            ...
            using (SqlCommand qryCmd = new SqlCommand(qry, con))
            {
                ...
                {                    
                    ...
                    Console.WriteLine(temp_data[0].ToString());

                    req.RequestID = int.Parse(temp_data[0].ToString());
                    req.RequesterNumber = string.Format("0{0}", SubscriberManagement.Domain.GeneralService.OCSFormatMSISDN(temp_data[1].ToString()));
                    req.Network = int.Parse(temp_data[2].ToString());
                    req.Amount = double.Parse(temp_data[3].ToString());
                    req.ReceiverNumber = temp_data[4].ToString();
                    req.Status = int.Parse(temp_data[5].ToString());
            ...
            //con = new SqlConnection(conString);
            //string qry = "SELECT * FROM Temp_Recon WHERE Status=2";
            ...
            //    {
            //        RequestModel req = new RequestModel();
            //        req.RequestID = int.Parse(temp_data[5].ToString());
            //        req.RequesterNumber = string.Format("0{0}", SubscriberManagement.Domain.GeneralService.OCSFormatMSISDN(temp_data[1].ToString()));
            //        //  req.Network = int.Parse(temp_data[2].ToString());
            //        req.Amount = double.Parse(temp_data[3].ToString());
            //        // req.ReceiverNumber = temp_data[4].ToString();
